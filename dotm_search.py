...
"""
__author__ = "Tiffany McLean with the help of stackoverflow, python documentation, programiz, and tons of Google-fu"
...
def search_file(file_name, search_text, files_with_string):
    with zipfile.ZipFile(file_name, 'r') as zip:
        print(file_name)
        with zip.open("word/document.xml") as contents:
            read_contents = contents.read()
            found_string = 0
            for is_string in find_all(read_contents, search_text):
                found_string += 1
...
def loop_dir(args):
    ...
    for file in list_of_files:
        ...
        if is_path.endswith('.dotm'):
            search_file(is_path, args.text, files_with_string)
            file_count += 1
